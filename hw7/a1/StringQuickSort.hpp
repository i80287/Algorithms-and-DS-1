...
constexpr Iterator move_strings_to_left(Iterator begin, Iterator end,
    ...
    assert(insertion_pos_iter == begin || compare_func(*(insertion_pos_iter - 1)));
    assert(!compare_func(*insertion_pos_iter));
    ...
    for (++first_equal_iter; first_equal_iter != end;) {
        ...
        assert(insertion_pos_iter < first_equal_iter);
...
constexpr PartitionResult<Iterator> partition(Iterator begin, Iterator end, Iterator pivot_iter,
    ...
    begin = move_strings_to_left(begin, end, [=](const StringType& s) constexpr noexcept {
        assert(common_prefix_length < s.size());
...
struct StringQuickSortNiebloid final {
    static constexpr auto kAlgorithmName =
        niebloid_tools::get_algorithm_name<StringQuickSortNiebloid>();
