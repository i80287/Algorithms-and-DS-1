...
constexpr auto counting_sort_impl(const Iterator begin, const Iterator end,
    ...
    assert(size <= tmp_buffer.capacity());
    tmp_buffer.resize(size);

    for (auto iter = begin; iter != end; ++iter) {
        const StringType& str = *iter;
        assert(common_prefix_length < str.size());
...
#if defined(__GNUC__) && __GNUC__ >= 13 && defined(__clang__) && defined(__cpp_lib_ranges_zip) && \
...
struct MSDRadixSortNiebloid final {
    static constexpr auto kAlgorithmName =
        niebloid_tools::get_algorithm_name<MSDRadixSortNiebloid<kSwitchToQuickSort>>();
...
template <bool kSwitchToQuickSort = false>
