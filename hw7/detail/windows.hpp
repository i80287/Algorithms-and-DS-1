...
class ClipboardWindows final : public ClipboardInterface<ClipboardWindows> {
public:
    ...
    std::string paste() const noexcept {
        OpenCloseClipboardRaii clipboard_raii;
...
private:
    ...
    public:
        WindowsException(const std::string& reason)
            : exception(reason + " (" + get_last_windows_error() + ")"){};
    ...
    void empty_clipboard() const {
        WINBOOL succeed = EmptyClipboard();
        if (!succeed)
    ...
    void set_clipboard_data_from_memory(std::unique_ptr<char, WindowsPtrDeleter> buffer) const {
        if (SetClipboardData(CF_TEXT, buffer.get())) {
            // from now on the operating system owns the buffer
            (void)buffer.release();
        } else {
