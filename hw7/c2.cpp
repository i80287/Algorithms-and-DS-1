...
int main() {
    string p, t;
    p.reserve(size_t(1e3) + 3);
    t.reserve(size_t(1e7) + 3);
    cin >> p >> t;
    ...
    size_t pi[1000] = {};
    {
        ...
        for (size_t i = 1; i < n; i++) {
            size_t j = pi[i - 1];
            uint32_t c_i = uint8_t(p[i]);
            while (j > 0 && c_i != uint8_t(p[j])) {
                assert(j - 1 < n);
                j = pi[j - 1];
            }
            assert(j < n);
    ...
    for (size_t t_i = 0, p_i = 0; t_i < t_size; t_i++) {
        uint32_t c = uint8_t(t[t_i]);
        assert(p_i < p_size);
        // iters++;
        while (p_i > 0 && uint8_t(p[p_i]) != c) {
            // iters++;
            p_i = pi[p_i - 1];
        }
