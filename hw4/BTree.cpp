...
BTree::BTree(int t) {
    assert(t > 0);
    keys_capacity_ = 2 * size_t(uint32_t(t)) - 1;
    nodes_.reserve(64);
...
void BTree::insert(int key) {
    ...
    for (uint32_t child_index = 0;;) {
        ...
        uint32_t jostle_node_index = new_node();
        ...
        uint32_t new_root_index = new_node();
