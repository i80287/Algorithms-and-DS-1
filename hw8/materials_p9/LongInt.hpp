...
struct LongInt {
    ...
    void DivMod(const LongInt& other, LongInt& rem) {
        ...
        for (std::size_t j = m - n; static_cast<std::size_t>(j) >= 0; j--) {
    ...
    constexpr LongInt& operator>>=(uint32_t shift) noexcept {
        ...
        uint32_t* const nums_iter_last = nums_iter + std::size_t(usize) - 1;
    ...
    void set_string(std::string_view s) {
        ...
        std::size_t digits_count = static_cast<std::size_t>(str_end - str_iter);
        if (digits_count <= 19) {
            uint64_t num = 0;
...
private:
    ...
    static constexpr uint64_t LongIntAdd(uint32_t nums1[], const uint32_t nums2[], std::size_t usize1,
