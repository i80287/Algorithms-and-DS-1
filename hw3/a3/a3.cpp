...
inline constexpr std::string_view kRandomDataFileSuffix = "_random_arr.data";
inline constexpr std::string_view kReversedDataFileSuffix = "_reversed_arr.data";
inline constexpr std::string_view kAlmostSortedDataFileSuffix = "_sorted_arr.data";
...
vector<Measurement> MeasureSortingAlgorithm(SortingAlgo f,
    ...
    for (size_t n = kMinN; n <= kMaxN; n += kStepN) {
        ...
        for (size_t i = kEachMeasureLimit; i != 0; i--) {
            ...
            // Just prevent compiler from optimizing away sorting algorithm
            // call.
...
inline std::vector<Result> MeasureSortingAlgorithms() {
    ...
    return {MeasureSortingAlgorithm(QuickSort<Iter>, rand_vec, rev_sorted_vec,
                                    almost_sorted_vec),
...
int32_t PlotData(const char data_filename[], const char title[],
                 const char color[]) {
    ...
    fprintf(gnupipe,
            ...
            title, data_filename, title, color);
...
int32_t PlotAllData(std::string_view filename_prefix,
    ...
    title += ", random array";
    res |= PlotData(data_filename.c_str(), title.c_str(), color.data());
...
int main() {
    ...
    std::string_view kArgs[][3] = {
        {"quick_sort", "Quick sort without cutoff", "red"},
